...
;PmRealVector*(*mInputValues)(const NeslSimulationData*);PmIntVector*(*
mInputOffsets)(const NeslSimulationData*);boolean_T*(*mFoundZcEvents)(const
...
;void(*mDestroy)(NeslSimulationData*);NeslSimulationDataData*mData;};
PMF_DEPLOY_STATIC PmRealVector nesl_get_time(const NeslSimulationData*sd){
return*(sd->mTime(sd));}PMF_DEPLOY_STATIC PmRealVector nesl_get_cont_states(
const NeslSimulationData*sd){return*(sd->mContStates(sd));}PMF_DEPLOY_STATIC
PmIntVector nesl_get_mode_vector(const NeslSimulationData*sd){return*(sd->
mModeVector(sd));}PMF_DEPLOY_STATIC PmRealVector nesl_get_disc_states(const
NeslSimulationData*sd){return*(sd->mDiscStates(sd));}PMF_DEPLOY_STATIC
PmIntVector nesl_get_sample_hits(const NeslSimulationData*sd){return*(sd->
mSampleHits(sd));}PMF_DEPLOY_STATIC PmRealVector nesl_get_outputs(const
...
PMF_DEPLOY_STATIC PmRealVector nesl_get_tolerances(const NeslSimulationData*sd
){return*(sd->mTolerances(sd));}PMF_DEPLOY_STATIC PmSparsityPattern
...
->mSolJacobianPr(sd));}PMF_DEPLOY_STATIC PmRealVector nesl_get_input_values(
const NeslSimulationData*sd){return*(sd->mInputValues(sd));}PMF_DEPLOY_STATIC
...
NeslSimulationData*,NeuDiagnosticManager*);typedef void(*
NeslSimulatorInitializeStartStatus)(void*);typedef void(*
NeslSimulatorInitializeEndStatus)(void*);struct NeslSimulatorTag{
...
NE_PARTITIONING_METHOD_ALL_EQS,NE_PARTITIONING_METHOD_NUM}NePartitionMethod;
...
NE_INTEG_EQ_FLAG=1<<6,NE_NO_EQ_FLAGS=0};enum{NE_INITIAL_ICR_FLAG=1<<0,
NE_NO_ICR_FLAGS=0};enum{NE_NODAL_VAR_FLAG=1<<0,NE_OUTPUT_VAR_FLAG=1<<1,
NE_INPUT_VAR_FLAG=1<<2,NE_SIGNAL_VAR_FLAG=1<<3,NE_TIME_VAR_FLAG=1<<4,
NE_INTEG_VAR_FLAG=1<<5,NE_NO_VAR_FLAGS=0};typedef union NeDsMethodOutputTag{
PmSparsityPattern mM_P;PmRealVector mM;PmBoolVector mVMM;PmSparsityPattern
mDXM_P;PmRealVector mDXM;PmSparsityPattern mDDM_P;PmRealVector mDDM;
PmSparsityPattern mDUM_P;PmRealVector mDUM;PmSparsityPattern mDTM_P;
PmRealVector mDTM;PmSparsityPattern mA_P;PmRealVector mA;PmSparsityPattern mB_P
;PmRealVector mB;PmSparsityPattern mC_P;PmRealVector mC;PmRealVector mF;
PmBoolVector mVMF;PmBoolVector mSLF;PmBoolVector mSLF0;PmSparsityPattern mDXF_P
...
PmRealVector mDDF;PmSparsityPattern mDWF_P;PmRealVector mDWF;PmSparsityPattern
...
NE_DS_METHOD_DDF_P,NE_DS_METHOD_DDF,NE_DS_METHOD_DWF_P,NE_DS_METHOD_DWF,
...
NE_DS_METHOD_LOG,NE_DS_METHOD_ASSERT,NE_DS_METHOD_PASSERT,NE_DS_METHOD_IASSERT
...
SLC_NUM_METHODS];size_t mMNnz;size_t mJNnz;boolean_T isLti;
...
*mStart)(const Simulator*solver,const NeParameterBundle*bundle);
...
NeObservableData*obsdata,const size_t numObservables);
#ifdef __cplusplus
}
#endif /* __cplusplus */
